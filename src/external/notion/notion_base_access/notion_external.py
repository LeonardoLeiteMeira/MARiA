...
class NotionExternal:
    ...
    async def get_page(self, page_id: str) -> Any:
        data = await self.notion_client.pages.retrieve(**{"page_id": page_id})
        return data
        ...
        page["children"] = [
            {
                "object": "block",
                "heading_1": {
                    "rich_text": [{"text": {"content": "Pagina criada por MARiA"}}]
                },
            },
        ]
    ...
    async def process_datasource_registers(
        self, data: dict[str, Any]
    ) -> dict[str, Any]:
        full_data = {"has_more": data["has_more"], "next_cursor": data["next_cursor"]}
        registers = []
    ...
    async def process_page_register(self, page: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in page["properties"].items():
            ...
            if property.property_type == "relation":
                row[property.key] = [
                    await self.__get_page_name(page_id["id"])
                    for page_id in property.value
                ]
        return row
        ...
        for result in search_result["results"]:
            datasources.append(
                NotionBaseDatasource(
                    name=result["title"][0]["text"]["content"], id=result["id"]
                )
            )
        return datasources
