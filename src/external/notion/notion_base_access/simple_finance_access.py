...
class SimpleFinanceAccess(BaseTemplateAccessInterface):
    ...
    async def create_new_transaction(
        ...
        properties = {
            ...
            **({"Tipo Saida": {"relation": [{"id": macro_category_id}]}} if macro_category_id is not None else {}),
            **({"Valor": {"number": amount}} if amount is not None else {}),
            **({"Criado em": {"date": {"start": date}}} if date is not None else {}),
            **({"Tipo Transação": {"select": {"name": template_transaction_type.value}}} if template_transaction_type is not None else {})
        }
        page = {
    ...
    def __convert_global_transaction_type(self,transaction_type: GlobalTransactionType) -> TransactionType:
        match(transaction_type):
            ...
            case GlobalTransactionType.PAY_CREDIT_CARD:
                return TransactionType.PAY_CREDIT_CARD
