...
class PluggyItemRepository(BaseRepository):
    async def get_pluggy_item_by_item_id(
        self, item_id: uuid.UUID
    ) -> PluggyItemModel | None:
        stmt = select(PluggyItemModel).where(PluggyItemModel.id == item_id)
    ...
    async def create_transactions(
        self, transactions: list[PluggyTransactionModel]
    ) -> None:
        async with self.session() as session:
    ...
    async def create_investment_transactions(
        self, transactions: list[PluggyInvestmentTransactionModel]
    ) -> None:
        async with self.session() as session:
            session.add_all(transactions)
            await session.commit()
    ...
    async def get_investments_by_user(
        self, user_id: uuid.UUID
    ) -> list[PluggyInvestmentModel]:
        stmt = select(PluggyInvestmentModel).where(
            PluggyInvestmentModel.user_id == user_id
        )
        async with self.session() as session:
            cursor = await session.execute(stmt)
            return list(cursor.scalars().all())
