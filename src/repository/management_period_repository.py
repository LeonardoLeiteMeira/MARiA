...
class ManagementPeriodRepository(
    BaseRepository, ManagementPeriodFilterToSqlAlchemyMixin
):
    ...
    async def update(self, management_period: ManagementPeriodModel) -> None:
        ...
        data = {
            ...
            and getattr(management_period, column) is not None
        }
    ...
    async def delete(self, management_period: ManagementPeriodModel) -> None:
        if management_period.id is None:
            raise Exception("management period id is not defined")
        stmt = delete(ManagementPeriodModel).where(
            ManagementPeriodModel.id == management_period.id,
            ManagementPeriodModel.user_id == management_period.user_id,
        )
        async with self.session() as session:
    ...
    async def get_by_id(
        self, management_period_id: uuid.UUID
    ) -> ManagementPeriodModel | None:
        stmt = select(ManagementPeriodModel).where(
            ManagementPeriodModel.id == management_period_id
        )
        async with self.session() as session:
    ...
    async def get_by_ids(
        self, management_period_ids: Sequence[uuid.UUID], user_id: uuid.UUID
    ...
    async def get_by_filter(
        ...
        async with self.session() as session:
            ...
            management_period_list_dto = [
                ManagementPeriodDto.model_validate(model)
                for model in management_period_list
            ]
