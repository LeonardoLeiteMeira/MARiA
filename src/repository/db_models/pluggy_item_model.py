...
class PluggyItemModel(BaseModel):
    ...
    connector: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False)
    products: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False)
    complementary_data: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False)
    ...
    def from_request_body(cls, data: dict[str, Any], user_id: str) -> "PluggyItemModel":
        ...
        item.last_updated_at = datetime.strptime(
            data["item"]["lastUpdatedAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        item.created_at = datetime.strptime(
            data["item"]["createdAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        item.updated_at = datetime.strptime(
            data["item"]["updatedAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
