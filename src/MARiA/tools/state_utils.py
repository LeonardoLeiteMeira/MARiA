...
def get_state_records_by_type(
    state: State, data_type: UserDataTypes
) -> list[dict[str, Any]]:
    return get_state_records(state, STATE_KEY_BY_TYPE.get(data_type))


def get_data_id_from_state(
    state: State, data_type: UserDataTypes, register_name: str | None
) -> str | None:
